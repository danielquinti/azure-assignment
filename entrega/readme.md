...
> **⚠️ Importante:** La sintaxis de los comandos y variables detallada a lo largo de esta memoria está diseñada para su ejecución en **PowerShell** (Windows). Si utilizas un sistema basado en Linux o macOS, deberás adaptar la sintaxis de las variables de entorno (ej. usar `export VAR="valor"` en lugar de `$env:VAR="valor"`) y comandos de red (ej. usar `curl` en lugar de `Invoke-WebRequest`).
...
* **Verificación:** Ejecuta el siguiente comando en PowerShell para verificar que el servicio está activo:
  ...
  ![Verificación de Docker Desktop](images/01_docker.png)
...
* **Verificación:** Ejecuta el siguiente comando en PowerShell para confirmar que está instalado correctamente:
  ...
  ![Verificación de Azure CLI](images/02_az.png)
...
![Diagrama de Secuencia - Arquitectura Mínima v1](images/03_diagrama_arquitectura_v1.png)
...
![Arranque de contenedor único](images/04_arranque_contenedor_unico.png)
...
![Prueba local exitosa](images/05_prueba_local_contenedor_unico.png)
...
![Autenticación en Azure](images/06_login.png)
...
![Definición de variables de entorno](images/07_variables.png)
...
![Grupo de Recursos](images/08_creacion_grupo_recursos.png)
...
![Azure Container Registry](images/09_creacion_acr.png)
...
![Habilitar Administrador ACR](images/10_habilitar_admin_acr.png)
...
![Autenticación Docker local](images/11_login_docker_acr.png)
...
![Construcción de la imagen Docker](images/12_build_imagen_docker.png)
...
![Subida de la imagen a ACR](images/13_push_imagen_acr.png)
...
![Creación del Plan de App Service](images/14_creacion_plan_appservice.png)
...
![Creación de la Web App en Azure](images/15_creacion_webapp_azure.png)
...
![Configuración de la Web App en Azure](images/16_configuracion_webapp_azure.png)
...
![Verificación de la API en Azure](images/17_verificacion_api_azure_v1.png)
...
![API Key leída localmente](images/18_gemini_compose.png)
...
![Prueba local con Docker Compose](images/19_prueba_local_docker_compose.png)
...
![Variables de entorno cargadas para compose](images/20_variable_compose.png)
...
![Autenticación en el ACR para Compose](images/21_acr_compose.png)
...
![docker-compose build](images/22_build_imagenes_compose.png)
![docker-compose push](images/23_push_imagenes_compose.png)
...
![Creación del Plan de App Service](images/24_creacion_plan_appservice_compose.png)
![Creación de la Web App Multi-Contenedor](images/25_creacion_webapp_multicontenedor.png)
...
![Configuración del Contenedor Multi-Contenedor](images/26_configuracion_contenedor_multicontenedor.png)
![Configuración de Variables de Entorno Multi-Contenedor](images/27_configuracion_variables_multicontenedor.png)
...
![Verificación de la API Multi-Contenedor en Azure](images/28_verificacion_api_azure_compose.png)
...
![Diagrama de Secuencia - Multi-Contenedor v2](images/29_diagrama_arquitectura_v2.png)
...
![Definición de variables de entorno adicionales](images/30_variables.png)
...
![Infraestructura de Almacenamiento](images/31_creacion_storage_account_fileshare.png)
...
![Obtención de la clave de almacenamiento](images/32_variable.png)
...
![Conexión de Almacenamiento](images/33_montaje_storage_webapp.png)
...
![Prueba de Historial en Local](images/34_prueba_historial_local.png)
...
![Prueba de Persistencia en Local](images/35_prueba_persistencia_local.png)
...
1. Reconstruye y sube las imágenes al ACR con los nuevos cambios de código:
   ...
   ![Compilación y subida de imágenes](images/36_build.png)
...
2. Actualiza la configuración de la Web App con el nuevo `docker-compose.yml` (que incluye el volumen) y fuerza el reinicio:
   ...
   ![Configuración del contenedor en Azure](images/37_config.png)
...
4. Consulta el endpoint de historial para comprobar que ha registrado la consulta:
   ...
   ![Consulta de historial en Azure](images/38_history.png)
...
5. (Opcional) Verifica la persistencia listando los archivos en Azure File Share directamente mediante Azure CLI:
   ...
   ![Verificación opcional de persistencia en Azure Files](images/39_opcional.png)
...
7. Vuelve a realizar la consulta al historial tras esperar un par de minutos:
   ...
   **Resultado:** El endpoint debe seguir devolviendo `{"history":["Azure_Prompt_1"]}` demostrando que el archivo se almacena en el volumen persistente de Azure Files y no se pierde tras reiniciar el servicio.
   ...
   ![Prueba de Persistencia en Azure](images/40_prueba_persistencia_azure.png)
...
![Diagrama de Arquitectura Final](images/41_diagrama_arquitectura_v3.png)
...
1. Lee la API Key de Gemini desde el archivo `.env` local:
   ...
   ![Carga de variables locales](images/42_env_local.png)
...
3. Verifica la traza de logs de comunicación a través del endpoint `/logs`:
   ...
   **Salida esperada (en formato JSON):**
   ...
   ![Prueba de comunicación y logs en local](images/43_prueba_comunicacion_logs_local.png)
...
6. Consulta nuevamente el endpoint de logs:
   ...
   **Resultado:** El endpoint debe seguir devolviendo los registros de logs previos (`"INFO - Enviando prompt..."` y `"INFO - Respuesta exitosa..."`), demostrando que el archivo se almacena en el volumen persistente `AlmacenamientoPersistente` en tu equipo local y sobrevive a la recreación de los contenedores.
   ...
   ![Persistencia de logs tras recreación de contenedores](images/44_persistencia_logs_tras_reinicio_local.png)
...
![Variables de entorno cargadas](images/45_vars.png)
...
![Creación del Grupo de Recursos y del Azure Container Registry](images/46_grupo.png)
...
![Creación de la Cuenta de Almacenamiento](images/47_storageacc.png)
...
![Creación del recurso compartido de archivos](images/48_share.png)
...
![Habilitación de usuario administrador en ACR](images/49_admin.png)
...
![Autenticación local en Docker](images/50_echo.png)
...
![docker-compose build y push de imágenes v3](images/51_build.png)
...
![Creación del Plan de App Service](images/52_appservice.png)
...
![Creación de la Web App en Azure](images/53_webapp.png)
...
![Configuración de la Web App en Azure](images/54_webapp_config.png)
...
![Configuración del almacenamiento persistente en Azure](images/55_configuracion_storage_azure_v3.png)
...
2. **Consulta del endpoint de logs para verificar la traza de comunicaciones internas:**
   ...
   **Salida esperada:**
   ...
   ![Logs de interconexión y peticiones en Azure](images/56_req_azure.png)
...
3. **(Opcional) Comprobar existencia física del log en Azure Files:**
   ...
   ![Existencia física de logs en Azure Files](images/57_check_storage.png)
...
4. **Prueba de persistencia de logs tras reinicio:**
   ...
   ![Prueba de persistencia de logs en Azure](images/58_prueba_persistencia_logs_azure.png)

> ℹ️ **Nota para la evaluación (Persistencia en la entrega):** Al realizar la primera consulta al endpoint `/logs` para evaluar el servicio, el profesor/evaluador observará que ya existe un historial almacenado en Azure Files. Este historial corresponde a las pruebas de verificación realizadas por el alumno durante el despliegue final (recogiendo tanto los reintentos y fallos de conexión temporales previos al arranque total del backend, como las posteriores llamadas exitosas a Gemini). Esto demuestra que los logs persisten de forma indefinida en la nube y sobreviven al ciclo de vida de los contenedores:
> ```json
> {
>   "logs": [
>     "2026-06-29 17:05:10,859 - INFO - Enviando prompt al servicio B en la URL http://backend:8080/generate",
>     "2026-06-29 17:05:11,088 - ERROR - Error de comunicación inesperado con servicio B: ConnectError('All connection attempts failed')",
>     "2026-06-29 17:05:46,394 - INFO - Enviando prompt al servicio B en la URL http://backend:8080/generate",
>     "2026-06-29 17:06:01,067 - INFO - HTTP Request: GET http://backend:8080/generate?prompt=Azure_Prompt_v3 \"HTTP/1.1 200 OK\"",
>     "2026-06-29 17:06:01,073 - INFO - Respuesta exitosa recibida del servicio B."
>   ]
> }
> ```
...
* [az storage file (Administración de archivos y directorios en Azure Files)](https://learn.microsoft.com/cli/azure/storage/file)

### Conceptos y Guías de Arquitectura en Azure:
* [Configurar una aplicación multi-contenedor personalizada en Azure App Service](https://learn.microsoft.com/azure/app-service/configure-custom-container?pivots=container-howto)
* [Montar Azure Storage como un directorio local en un contenedor de App Service](https://learn.microsoft.com/azure/app-service/configure-connect-to-azure-storage?pivots=container-linux)
...
* **Código de la cuarta versión (Bloque 4):** [Release v3 de GitHub](https://github.com/danielquinti/azure-assignment/releases/tag/v3)
